...
from utils import IMDB, data_generator
		...
		for epoch in range(hparam.num_epoch):
			...
			for b in tqdm(range(train_num_batches)):
				...
				lo, acc, _, summ = sess.run([model.loss, model.accuracy, model.train_op, model.merged],
					feed_dict={model.inputs:x_batch, model.targets: y_batch, model.lengths:seq_len, model.keep_prob:hparam.keep_prob})
				train_loss += lo
				train_accuracy += acc
			...
			for b in tqdm(range(test_num_batches)):
				...
				lo_test, acc_test, summ_test = sess.run([model.loss, model.accuracy, model.merged],
					feed_dict={model.inputs:test_x_batch, model.targets:test_y_batch, model.lengths:test_seq_len, model.keep_prob:1.0})
				test_loss += lo_test
